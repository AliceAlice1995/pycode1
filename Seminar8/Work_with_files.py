...
def create_table():   # создаем пустую таблицу со столбцами id, name, age если ее не было
...
con = sl.connect("gb.db")  # соединяем с БД, если ее нет, то создаем такую БД
cur = con.cursor()         # создаем указатель, через него будем выполнять запрос 
